var app = new Vue({
  ...
  computed: {
    ...
    game_xg() {
      ...
      xg_vals = {
        home: [{ min: 0, xg: 0, pid: null, info: null, side: "home" }],
        away: [{ min: 0, xg: 0, pid: null, info: null, side: "away" }],
      };
      for (let shot of game_data.content.shotmap.shots) {
        let side = !((shot.teamId == home_id) ^ !shot.isOwnGoal)
          ? "home"
          : "away";
    ...
    players_by_xg() {
      if (_.isEmpty(this.game_xg)) {
        return [];
      }
      let game_xg = _.cloneDeep(this.game_xg);
      let all_shots = game_xg["home"]
        .concat(game_xg["away"])
        .filter((i) => i.info);
      ...
      return _.orderBy(
        _.map(grouped, (i) => {
          return {
            values: i,
            total_xg: _.sumBy(i, "info.expectedGoals"),
            best_xg: _.maxBy(i, "info.expectedGoals").info.expectedGoals,
            shots: i.length,
            side: i[0].side,
            name: i[0].info.lastName,
          };
        }),
        "total_xg",
        "desc"
  ...
  methods: {
    set_colors() {
      ...
      if (Array.isArray(t)) {
        this.home_color_options.push(
          this.game_json.general.teamColors[0].color
        );
        this.home_color_options.push(
          this.game_json.general.teamColors[0].colorAlternate
        );
        this.home_color_options.push(
          this.game_json.general.teamColors[0].colorAway
        );
        this.home_color_options.push(
          this.game_json.general.teamColors[0].colorAwayAlternate
        );
        this.home_color = this.game_json.general.teamColors[0].color;

        this.away_color_options.push(
          this.game_json.general.teamColors[1].color
        );
        this.away_color_options.push(
          this.game_json.general.teamColors[1].colorAlternate
        );
        this.away_color_options.push(
          this.game_json.general.teamColors[1].colorAway
        );
        this.away_color_options.push(
          this.game_json.general.teamColors[1].colorAwayAlternate
        );
        this.away_color = this.game_json.general.teamColors[1].color;
      } else if (t?.homeColors) {
        debugger;
        this.home_color_options.push(
          this.game_json.general.teamColors.homeColors.color
        );
        this.home_color_options.push(
          this.game_json.general.teamColors.homeColors.colorAlternate
        );
        this.home_color_options.push(
          this.game_json.general.teamColors.homeColors.colorAway
        );
        this.home_color_options.push(
          this.game_json.general.teamColors.homeColors.colorAwayAlternate
        );
        this.home_color = this.game_json.general.teamColors.home;

        this.away_color_options.push(
          this.game_json.general.teamColors.awayColors.color
        );
        this.away_color_options.push(
          this.game_json.general.teamColors.awayColors.colorAlternate
        );
        this.away_color_options.push(
          this.game_json.general.teamColors.awayColors.colorAway
        );
        this.away_color_options.push(
          this.game_json.general.teamColors.awayColors.colorAwayAlternate
        );
        this.away_color = this.game_json.general.teamColors.away;
      } else {
        this.home_color_options.push(
          this.game_json.general.teamColors.lightMode.home
        );
        this.home_color = this.game_json.general.teamColors.lightMode.home;

        this.away_color_options.push(
          this.game_json.general.teamColors.lightMode.away
        );
...
function plot_game_xg() {
  ...
  plot_area
    ...
    .attr("class", "axis-holder")
    .call(d3.axisLeft(y).tickSize(25).tickValues(_.range(0, graph_max, 0.5)));
  ...
  svg
    ...
    .call((g) =>
      g.selectAll(".tick text").attr("font-size", "24pt").attr("fill", "black")
    );
  ...
  if (hp != undefined) {
    let all_shots = _.cloneDeep(
      xg_vals["home"].concat(xg_vals["away"]).filter((i) => i.info)
    );
  ...
  let single_goal = plot_area
    ...
    .data((d) => {
      return xg_vals[d]
        .filter((i) => i.info?.eventType == "Goal")
        .map((i) => {
          return { ...i, side: d };
        });
    })
  ...
  single_goal
    ...
    .html(function (d) {
      return `
            <div class="outer goal-text d-flex flex-column" contenteditable spellcheck="false">
                <span class="w-100 text-center box-name" style="color: ${side_color(
                  d.side
                )}">${d.info.lastName}</span>
                <span class="w-100 text-center box-value">${
                  d.info?.isOwnGoal
                    ? "(OG)"
                    : d.info?.expectedGoals?.toFixed(2) + " xG" || "0.00 xG"
                }</span>
            </div>
            `;
    })
  ...
  league_title =
    game_data.general.parentLeagueName +
    " " +
    game_data.general.parentLeagueSeason;
  date_title = game_data["matchTimeUTC"];
  overall_title =
    transformed(game_data.general.homeTeam.name) +
    " " +
    game_data.header.status.scoreStr +
    " " +
    transformed(game_data.general.awayTeam.name);

  // let logo_url = (v) => `https://api.allorigins.win/raw?url=${v}`
  let logo_url = (v) => `${proxy}/${v.replace("https://", "")}`;
                ...
                <div class="w-100 row no-gutters">
                    <div class="col text-right d-flex justify-content-end">
                        <div class="d-flex flex-column mr-10">
                            <span class="teamline" style="color: ${side_color(
                              "home"
                            )}">${transformed(game_data.general.homeTeam.name)}</span>
                            <span class="text-center xgline">${xg_vals.home
                              .at(-1)
                              .xg.toFixed(2)} xG</span>
                        </div>
                        <div class="crest"><img class="crestimg" id="home-image" src="${logo_url(
                          game_data.header.teams[0].imageUrl.replace(
                            "https://",
                            ""
                          )
                        )}" /></div>
                    ...
                    <div class="col text-left d-flex">
                        <div class="crest mr-10"><img class="crestimg" id="away-image" src="${logo_url(
                          game_data.header.teams[1].imageUrl.replace(
                            "https://",
                            ""
                          )
                        )}" /></div>
                        <div class="d-flex flex-column">
                            <span class="teamline" style="color: ${side_color(
                              "away"
                            )}">${transformed(game_data.general.awayTeam.name)}</span>
                            <span class="text-center xgline">${xg_vals.away
                              .at(-1)
                              .xg.toFixed(2)} xG</span>
                        </div>
                    </div>
  ...
  plot_area
    ...
    .style(
      "transform",
      `translate(-100px, ${plot_height / 2}px) rotate(-90deg)`
    );

  const bluesky_logo = `
<svg width="24" height="18" viewBox="0 0 600 520" version="1.1" xmlns="http://www.w3.org/2000/svg">
      ...
      return `
        <div class="outer text-center">
            <div class="text-center w-100 creditline">
                Viz by <span style="color: none;">${bluesky_logo}</span>@sertalpbilal.com | Data by OPTA
            </div>
        </div>
        `;
    });
...
$(document).ready(() => {
  ...
  Promise.all([fetch_game_json()]).then(() => {
    console.log("ready");
    app.$nextTick(() => {
      app.set_colors();
      app.plot_xg();
...
function dragged(event, d) {
  let matrix = new WebKitCSSMatrix(
    window.getComputedStyle(d3.select(this).node()).transform
  );
